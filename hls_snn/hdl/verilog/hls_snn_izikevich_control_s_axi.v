...
localparam
    ...
    ADDR_STATE_DATA_0        = 6'h18,
    ADDR_STATE_CTRL          = 6'h1c,
    ADDR_P_INPUT_DATA_0      = 6'h20,
    ADDR_P_INPUT_CTRL        = 6'h24,
    ADDR_OUTPUT_INDEXES_BASE = 6'h28,
    ADDR_OUTPUT_INDEXES_HIGH = 6'h2f,
    WRIDLE                   = 2'd0,
    WRDATA                   = 2'd1,
    WRRESP                   = 2'd2,
    WRRESET                  = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [0:0]                    int_state = 'b0;
    reg  [31:0]                   int_p_input = 'b0;
    // memory signals
...
assign RVALID  = (rstate == RDDATA) & !int_output_indexes_read;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_P_INPUT_DATA_0: begin
                    rdata <= int_p_input[31:0];
                end
            endcase
        end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_STATE_DATA_0)
            int_state[0:0] <= (WDATA[31:0] & wmask) | (int_state[0:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_P_INPUT_DATA_0)
            int_p_input[31:0] <= (WDATA[31:0] & wmask) | (int_p_input[31:0] & ~wmask);
    end
end
...
assign int_output_indexes_we1      = int_output_indexes_write & w_hs;
assign int_output_indexes_be1      = int_output_indexes_we1 ? WSTRB : 'b0;
