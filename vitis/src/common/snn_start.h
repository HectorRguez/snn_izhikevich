...
int run_ila_test()
{
	printf("Sending data for ILA testing to FPGA...\n");
	reset_clock();
	start_clock();
	/*
	...
	*/
	...
	// Generating and sending testing data
	uint32_t data = 0xAAAAAAAA;
	uint32_t *data_pointer = &data;
	u32 stream[1] = { (u32) data_pointer }; // 32-bit processor
	hw_send_axi_stream_burst(stream, 1, sizeof(data));

	stop_clock();
	printf("Test sent successfully in %.2f ms.\n", get_clock_ms());
	return 0;
}
...
int run_hw_network() {
	...
	// Send network to FPGA
  	...
  	for (t = 0, o_off = 0; t < RUN_STEPS; t++, o_off += AXI_OUTPUT_LENGTH) {
		...
		if(t % 10000 == 0)
		{
			printf("Step %ld run successfully\n", t);
		}
		stop_clock();
